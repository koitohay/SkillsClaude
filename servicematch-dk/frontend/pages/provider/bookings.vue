<template>
  <div class="max-w-3xl mx-auto">
    <div class="mb-8">
      <h2 class="text-2xl font-bold text-gray-900">Mine bookinger</h2>
      <p class="text-sm text-gray-500 mt-1">Accepterede tilbud fra klienter</p>
    </div>
    ...
    <div v-else-if="!bookings.length" class="card text-center py-16">
      ...
      <p class="text-sm text-gray-500">Når en klient accepterer dit tilbud, vises det her.</p>
    </div>

    <div v-else class="space-y-4">
      <div v-for="booking in bookings" :key="booking.id" class="card hover:shadow-md hover:border-emerald-200 transition-all">
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="text-right flex-shrink-0">
            <div v-if="myAcceptedPrice(booking)" class="mb-3">
              <p class="text-xs text-gray-400">Dit tilbud</p>
              <p class="text-xl font-bold text-emerald-600">{{ myAcceptedPrice(booking) }} <span class="text-sm font-normal text-gray-500">DKK</span></p>
            </div>
            <NuxtLink :to="`/provider/requests/${booking.id}`" class="btn-ghost text-xs">
              Se detaljer →
            </NuxtLink>
          </div>
        </div>
...
const { data: allRequests, pending } = await useAsyncData('provider-requests-bookings', () =>
  $api<ServiceRequestDto[]>('/providers/me/requests'))

const bookings = computed(() =>
  (allRequests.value ?? []).filter(r => r.status === 'Accepted'))

function myAcceptedPrice(req: ServiceRequestDto): string | null {
  const offer = req.offers?.find(
    o => o.status === 'Accepted' && o.serviceProviderId === authStore.userId)
